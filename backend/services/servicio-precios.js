...
class ServicioPrecios {
  constructor() {
    ...
    this.TARIFA_POR_KM = 3.5; // S/ 3.5 por kilómetro
    this.TARIFA_POR_MIN = 0.6; // S/ 0.60 por minuto
    this.TARIFA_MINIMA = 10.0; // Tarifa mínima del viaje
    ...
    this.MULTIPLICADOR_MADRUGADA = 1.5; // 11pm-5am
    this.MULTIPLICADOR_ZONA_TURISTICA = 1.2;
    this.DESCUENTO_MOTOTAXI = 0.75; // Mototaxis 25% más baratos
  }
  ...
  async calcularPrecioSugerido(solicitudViaje) {
    ...
    if (await this.esZonaTuristica(origen_lat, origen_lon)) {
